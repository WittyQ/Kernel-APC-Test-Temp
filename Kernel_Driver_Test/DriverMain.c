...
//
//extern BOOLEAN is_notepad(void);
//
...
//				if (apc)
//				{
//					if (Global.os_major == 10 && Global.os_build < WBN_WIN10_MAY2021UPDATE)
//					{
//						KeInitializeApc(apc,
//							threadobj,
//							0,
...
//
//		if (bIsDwm) {
//			g_bDWMAttached = FALSE;//set flag to request restart dwm.exe when nxrmserv starts.
//		}
//
//	} while (FALSE);
//
...
NTSTATUS GetProcessIdByName(SYSTEM_PROCESS_INFORMATION * pProcessInfo) {
...
static BOOLEAN find_usermode_xxx(SYSTEM_PROCESS_INFORMATION* pProcessInfo, PVOID* pfn_LoadLibraryW)
{
	BOOLEAN bRet = TRUE;
...
NTSTATUS QueryLoadLibraryAdress(SYSTEM_PROCESS_INFORMATION* pProcessInfo) {
	NTSTATUS status = STATUS_SUCCESS;
	const WCHAR kernel32dll_path[256];
	const WCHAR ntdll_path[256];
	memset(kernel32dll_path, 0, sizeof(kernel32dll_path));

	RtlStringCbCatW(kernel32dll_path, sizeof(kernel32dll_path), (WCHAR*)(0x7ffe0030));
	RtlStringCbCatW(ntdll_path, sizeof(ntdll_path), (WCHAR*)(0x7ffe0030));

	RtlStringCbCatW(kernel32dll_path, sizeof(kernel32dll_path), L"\\System32\\kernel32.dll");
	RtlStringCbCatW(ntdll_path, sizeof(ntdll_path), L"\\System32\\ntdll.dll");
	find_usermode_xxx(pProcessInfo, &g_fn_LoadLibraryW);
